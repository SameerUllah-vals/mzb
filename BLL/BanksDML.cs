    ...
    public class BanksDML
    {
        public DataTable GetBank()
        {
            ...
            try
            {
                ...
                _commnadData.CommandText = "SELECT *, Name + '|' + AccountNo AS BankAccount FROM Banks ORDER BY Name ASC;";
        ...
        public DataTable GetBank(string Keyword)
        {
            ...
            {
                _commnadData._CommandType = CommandType.Text;
                _commnadData.CommandText = "Select * from Banks b  where ISNULL(b.isDelete,0)=0 And( [Code] like '%" + Keyword + "%' or [Name] like '%" + Keyword + "%'  or [AccountNo] like '%" + Keyword + "%' or [AccountTitle] like '%" + Keyword + "%'  ) ";
        ...
        public int DeleteBank(Int64 BankID, Int64 ModifiedByID)
        {
            ...
            {
                commandData._CommandType = CommandType.Text;
                commandData.CommandText = "UPDATE Banks SET [isDelete] = 'True' ,  [ModifiedByUserID] = '" + ModifiedByID + "', [DateModified] = getdate() WHERE [BankID] = " + BankID;
